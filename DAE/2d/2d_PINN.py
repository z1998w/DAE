# -*- coding: utf-8 -*-
"""
Created on Sun Apr  6 22:28:40 2025
...
def u_init(x, y):#先不用精初
    return 3 * torch.tanh(x / 0.01 + y) - 1
...
def compute_loss(model, x_internal, y_internal, t_internal, x_boundary, y_boundary, t_boundary, x_initial, y_initial, t_initial):
    ...
    pde_residual = 0.01 * (u_xx + u_yy) - u_t + u_internal * (u_x + u_y) - f
    ...
    loss_boundary = ((u_left - (-4)) ** 2+(u_right - 2) ** 2+(u_bottom - u_top) ** 2).mean()
    #两种写法是一样的
...
def train(model, epochs, optimizer):
    # ========== 原始版本 (LHS采样) ==========
    # 内部点
    x_internal = (torch.rand(3000, 1) * 4 - 2).to(device) 
    y_internal = (torch.rand(3000, 1) * 4 - 2).to(device) 
    t_internal = (torch.rand(3000, 1)).to(device) 
    
    # 边界点
    x_boundary = (torch.rand(3000//3, 1) * 4 - 2).to(device)#这个数量需要改,这样边界用了1500个点
    y_boundary = (torch.rand(3000//3, 1) * 4 - 2).to(device)
    t_boundary = (torch.rand(3000//3, 1)).to(device)
    
    # 初始点
    x_initial = (torch.rand(3000, 1) * 4 - 2).to(device)    # [-2, 2)
    y_initial = (torch.rand(3000, 1) * 4 - 2).to(device)    # [-2, 2)
    t_initial = torch.zeros(3000, 1).to(device)             # t=0
...
loss_history,newtime=train(model, 30000, optimizer)
...
# 读取真解数据
file_path = '2d_0410newdatamu2_ae_01_01_01.txt'
...
def plot_U0(x, y, U0):
    ...
    fig = plot_3d_surface(X, Y, Z, 
                          "Predicted Solution of PINN (t=0.2)",
                          cmap='plasma',
                          elev=25, 
                          azim=135)
    ...
    # 保存和显示
    plt.tight_layout()
    plt.savefig('2d_PINN_mu2_solution_t_0_2.eps', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_true_solution(x, y, u):
    ...
    # 保存和显示
    plt.tight_layout()
    plt.savefig('2d_PINN_mu2_true_solution_t_0_2.eps', dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_error(x, y, error):
    ...
    # ========== 标题和布局优化 ==========
    plt.title('Absolute Error of PINN (t=0.2)', 
              fontsize=28, 
              pad=25,
              fontweight='bold')
