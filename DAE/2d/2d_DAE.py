# -*- coding: utf-8 -*-
"""
Created on Thu Mar 27 19:30:27 2025
...
def phi_minus(x, y):
    ...
    return - (2 / torch.sqrt(3 * pi)) * torch.sqrt(
        torch.sin(pi * (x + y) / 4) -
        torch.sin(pi * (x - 2 * y - 6) / 4) +
        3 * torch.sin(pi * (x - y) / 4) -
        3 * torch.sin(pi * (x - 2 * y - 2) / 4) +
        12 * pi
    )
...
def phi_plus(x, y):
    ...
    return (2 / torch.sqrt(3 * pi)) * torch.sqrt(
        ...
        3 * torch.sin(pi * (x - 2 * y + 2) / 4) +
        3 * torch.sin(pi * (x - y) / 4) -
        torch.sin(pi * (x - 2 * y + 6) / 4) + 3 * pi
    )

def pde_loss(net, x, t):
    ...
    dh0_dx = grad(h0, x, grad_outputs=torch.ones_like(h0), create_graph=True)[0]
    
    # 计算PDE残差
    phi_m = phi_minus(x, h0)
    phi_p = phi_plus(x, h0)
    residual = dh0_dt - (dh0_dx - 0.5) * (phi_m + phi_p)
    return torch.mean(residual**2)

# def periodic_boundary_condition_loss(net, x, t, L=4):
#     x_L = x + L
#     x_L = torch.remainder(x_L + 2, L) - 2  # 周期性映射
#     h0_x = net(torch.cat([x, t], dim=1))
#     h0_x_L = net(torch.cat([x_L, t], dim=1))
#     return torch.mean((h0_x - h0_x_L)**2)
# def periodic_boundary_condition_loss(net, x, t, L=4):#把周期值固定对比两个代码哪个结果好
#     x_L = x + L
#     # x_L = torch.remainder(x_L + 2, L) - 2  # 周期性映射#用这个误差到3.9了
#     h0_x = t*net(torch.cat([x, t], dim=1))
#     h0_x_L = t*net(torch.cat([x_L, t], dim=1))
#     return torch.mean((h0_x - h0_x_L)**2)
def periodic_boundary_condition_loss(net, t):#用这个误差到4了
    # 定义边界点 x = -2 和 x = 2
    x_left = torch.full_like(t, -2.0)  # x = -2
    x_right = torch.full_like(t, 2.0)  # x = 2
    
    # 计算 h(-2, t) 和 h(2, t)
    h_left = t*net(torch.cat([x_left, t], dim=1))
    h_right = t*net(torch.cat([x_right, t], dim=1))
...
def train(net, epochs):
    ...
    # 内部点（2000个样本）
    x_internal = (torch.rand(2000, 1) * 4 - 2).to(device)  # x ∈ [-2, 2)
    t_internal = torch.rand(2000, 1).to(device)            # t ∈ [0, 1)
       
    # 边界点（1000个样本）
    x_boundary = (torch.rand(1000, 1) * 4 - 2).to(device)
    t_boundary = torch.rand(1000, 1).to(device)
    
    start_time = time.time()
    for epoch in range(epochs):
        # 使用固定采样点计算损失
        loss_pde = pde_loss(net, x_internal, t_internal)
    ...
    np.savez('2d_loss_history.npz', loss_history)
...
with torch.no_grad():
        # 组合时空坐标 (N, 2)
    XT_tensor = torch.tensor(np.hstack((X_flat, T_flat)), 
                                dtype=torch.float32).to(device)
        
    # 将 T_flat 转换为PyTorch张量
    T_flat_tensor = torch.tensor(T_flat, dtype=torch.float32).to(device)
    
    # 计算 h0_pred = T_flat * net(XT_tensor)（张量乘法）
    h0_pred = T_flat_tensor * net(XT_tensor)
        # 转换为numpy数组并重塑
    h0_pred = h0_pred.cpu().numpy().reshape(X.shape)  # (100, 100)
...
#     rstride=5 表示在行方向（Y轴）每隔5个点取一个数据点

# cstride=5 表示在列方向（X轴）每隔5个点取一个数据点,所以是1的时候才能显示出真实的数据
    # surf = ax.plot_surface(X, Y, Z, 
    #                       cmap=cmap,
    #                       rstride=5, 
    #                       cstride=5,
...
# 设置 3D 轴面板背景透明
    ...
    return fig
# # 绘制预测解的3D图h的角度有可能要调
fig_pred_3d = plot_3d_surface(X, T, h0_pred, 
                              "Predicted $h_0(x,t)$",xlabel="x", ylabel="t", zlabel="h")
plt.savefig('2d_DAE_x0_3d_surface.eps', dpi=300, bbox_inches='tight')
...
def plot_U0(x, y, U0,file):
    ...
    fig = plot_3d_surface(X, Y, Z, 
                          "Predicted Solution of DAE (t=0.5)",xlabel="x", ylabel="y", zlabel="u",
                          cmap='plasma',
                          elev=25, 
                          azim=135)
...
def plot_true_solution(x, y, u,file):
    ...
    # 使用统一绘图函数
    fig = plot_3d_surface(X, Y, Z, 
                          "Reference Solution (t=0.5)",xlabel="x", ylabel="y", zlabel="u",
                          cmap='plasma',
                          elev=25,
                          azim=135)
...
# 绘制结果
plot_U0(x_t_0_7, y_t_0_7, U0_t_0_7_1,'2d_DAE_mu2_solution_t_0_5.eps')
plot_true_solution(x_true_t_0_7, y_true_t_0_7, true_u_t_0_7_1,'2d_mu2_true_solution_t_0_5.eps')

# 绘制结果
plot_U0(x_t_0_7, y_t_0_7, U0_t_0_7_2,'2d_DAE_mu3_solution_t_0_5.eps')
plot_true_solution(x_true_t_0_7, y_true_t_0_7, true_u_t_0_7_2,'2d_mu3_true_solution_t_0_5.eps')

# 绘制结果
plot_U0(x_t_0_7, y_t_0_7, U0_t_0_7_3,'2d_DAE_mu4_solution_t_0_5.eps')
plot_true_solution(x_true_t_0_7, y_true_t_0_7, true_u_t_0_7_3,'2d_mu4_true_solution_t_0_5.eps')
...
def plot_error(x, y, error,file):
    ...
    # ========== 标题和布局优化 ==========
    plt.title('Absolute Error of DAE (t=0.5)', 
              fontsize=28, 
              pad=25,
              fontweight='bold')
